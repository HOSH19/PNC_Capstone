...
### 3b. champion으로 코퍼스 재라벨 ✔ 완료 2026-08-09

champion은 **클래스별 앙상블**이다: `negative`는 프롬프트 v4, 나머지는 전부
v3(`labels_ensemble_full.csv`). 두 프롬프트를 8,360행 전량에 돌렸고, 각각
300행 파일럿을 정확히 재현해 결정성이 확인됐다. 앙상블은 두 부모를 모두
앞선다 — kappa 0.683, macro-F1 0.828, `negative` 정밀도 0.923 / recall 0.857 —
그리고 홀드아웃만 봐도 유지된다(0.833 / 0.833 대 v3의 1.000 / 0.667). dev에
맞춘 조합이 아니다.

`positive` recall(0.676, 기준 0.70)은 여전히 미달이며 그대로 쓴다 —
`scoring/DESIGN.md` § "기준을 통과하지 못한 champion으로 학습하는 결정" 참조.

**의도적으로 받아들인 한계.** 학습셋의 `negative`는 **38행**이다. v2의 186행은
대부분 틀린 것이었고(정밀도 0.361), 2026년만 담은 코퍼스를 제대로 라벨링하면
그 이상 나오지 않는다. 2020~2024 백필을 라벨링하면 늘겠지만 학습 데이터와
백테스트 구간 사이의 누수 없는 분리가 깨지므로 기각했다. 모델의 `negative`
...
**완료 2026-08-14.** `pipeline/backfill_gkg.py`로 407,032행 적재(DOC API가 아니라
BigQuery GKG — 누적 요청량 제한 때문에 어떤 간격으로도 3,000요청을 거부한다).
실시간 수집도 같은 곳으로 옮겼다(`pipeline/poll_gkg.py`). 재학습은 확인 후
불필요로 판단했다 — 라벨러가 GKG형 텍스트에서 오히려 낫다(kappa 0.678 대 0.486).
다만 gold slice가 여전히 DOC API 표본이라, 게이트는 라벨러를 재되 코퍼스를
표집하지는 못한다.
