...
Human labels: `evals/items/gold_slice_*.csv` (300 rows). Llama labels: `labels_v4_pilot.csv` (`label_source='llama_kaggle'`, prompt v2).

## Headline (random sample, slices 1–5)

Overall agreement **238/250 = 95.2%** — placeholder threshold ≥85%: **PASS** on the letter of the threshold. **Read the next section before acting on that word** — the threshold sits below the do-nothing baseline, so passing it carries no information.
...
|---|---|---|
| raw agreement | 0.952 | 0.910 |
| all-neutral baseline | 0.916 | 0.840 |
| **Cohen's kappa** | 0.619 | 0.601 |
| macro F1 | 0.738 | 0.766 |

**The gate threshold is set below the trivial baseline.** On the random sample, 91.6% of human labels are `neutral`, so a labeler that answers `neutral` to every row scores 91.6% — above both the ≥85% threshold and Llama's own 95.2%. Raw agreement cannot distinguish a working labeler from a broken one on this corpus; the team's threshold decision should be restated in kappa or macro-F1.

Kappa 0.619 (random) / 0.601 (pooled) is *moderate* agreement on the conventional reading — not a failure, not a pass.
...
|---|---|---|---|---|
| kappa (primary) | ≥0.60 | 0.601 | 300 | meets |
| negative precision | ≥0.60 | 0.917 | 12 | meets |
| negative recall — guards the above | ≥0.85 | 0.786 | 14 | **below** |
| positive precision | ≥0.60 | 0.857 | 14 | meets |
| positive recall — guards the above | ≥0.70 | 0.353 | 34 | **below** |
| neutral recall — guards over-correction | ≥0.82 | 0.992 | 252 | meets |
...
|---|---|---|
| positive | 12/34 = 35.3% | 12/14 = 85.7% |
| negative | 11/14 = 78.6% | 11/12 = 91.7% |
| neutral | 250/252 = 99.2% | 250/274 = 91.2% |

**Key finding:** llama-side negative precision is 11/12 = 91.7% — when Llama says `negative`, humans usually see `neutral`. Llama over-calls negative; that is the systematic-bias pattern DESIGN.md asks this review to find.
...
|---|---|---|---|---|
| dev | 125 | 110/125 = 88.0% | 0.529 | 0.723 |
| holdout | 175 | 163/175 = 93.1% | 0.665 | 0.797 |
...
| slice | agreement |
|---|---|
| gold_slice_1 | 49/50 = 98.0% |
| gold_slice_2 | 47/50 = 94.0% |
| gold_slice_3 | 46/50 = 92.0% |
| gold_slice_4 | 48/50 = 96.0% |
| gold_slice_5 | 48/50 = 96.0% |
| gold_slice_6 | 35/50 = 70.0% (stratified — see headline note) |
...
| 15894 | 5 | gdelt | positive | neutral | Baystreet . ca - Goldman Sachs Posts Record Earnings And Raises Dividend 11 %  |
...
| 19244 | 2 | gdelt | positive | neutral | PNC Financial tops second - quarter forecasts as capital markets business drives growth ( PNC ) |
...
| 4759 | 3 | gdelt | negative | neutral | Betterment Faces Class Action Lawsuit Over Cash Sweep Accounts |
...
| 1919 | 6 | edgar | positive | neutral | WaFd, Inc. 8-K |
| 1408 | 6 | edgar | positive | neutral | Ally Financial Inc. 8-K |
| 1588 | 6 | edgar | positive | neutral | Banner Financial Corporation 8-K |
| 1575 | 6 | edgar | positive | neutral | Axos Financial, Inc. 8-K |
| 1784 | 6 | edgar | positive | neutral | OceanFirst Financial Corp. 8-K |
| 5844 | 6 | gdelt | positive | neutral | Bank of America Expands Regional Investment Banking Coverage , Adds Nine Key Senior Hires Across the U . S . - News |
| 1589 | 6 | edgar | positive | neutral | Banner Financial Corporation 8-K |
| 17656 | 6 | gdelt | neutral | positive | Fifth Third Bancorp Increases Holdings in GATX Corporation $GATX |
| 1585 | 6 | edgar | positive | neutral | BancFirst Corporation 8-K |
| 14336 | 6 | gdelt | positive | neutral | Morgan Stanley ( NYSE : MS ) Price Target Raised to $250 . 00 |
| 1593 | 6 | edgar | positive | neutral | BankUnited, Inc. 8-K |
| 3580 | 6 | edgar | positive | neutral | TriCo Bancshares 8-K |
| 6689 | 6 | gdelt | positive | neutral | Goldman Sachs , JPMorgan & BofA results tell a tale of stupendous times , squeezed costs |
