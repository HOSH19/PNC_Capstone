...
Human labels: `evals/items/gold_slice_*.csv` (300 rows). Llama labels: `labels_v3_pilot.csv` (`label_source='llama_kaggle'`, prompt v2).

## Headline (random sample, slices 1–5)

Overall agreement **229/250 = 91.6%** — placeholder threshold ≥85%: **PASS** on the letter of the threshold. **Read the next section before acting on that word** — the threshold sits below the do-nothing baseline, so passing it carries no information.

Slice 6 is a deliberate directional oversample (20 neg / 20 pos / 10 neu on Llama's label, own-bank titles only — see `export_new_gold_slice.py`), so it is excluded here; pooled over all 300 rows the number would read 271/300 = 90.3%, biased by sampling design, not labeler quality.

## Chance-corrected reading (the number that matters)

| | random (1–5) | pooled (1–6) |
|---|---|---|
| raw agreement | 0.916 | 0.903 |
| all-neutral baseline | 0.916 | 0.840 |
| **Cohen's kappa** | 0.497 | 0.650 |
| macro F1 | 0.717 | 0.808 |

**The gate threshold is set below the trivial baseline.** On the random sample, 91.6% of human labels are `neutral`, so a labeler that answers `neutral` to every row scores 91.6% — above both the ≥85% threshold and Llama's own 91.6%. Raw agreement cannot distinguish a working labeler from a broken one on this corpus; the team's threshold decision should be restated in kappa or macro-F1.

Kappa 0.497 (random) / 0.650 (pooled) is *moderate* agreement on the conventional reading — not a failure, not a pass.
...
|---|---|---|---|---|
| kappa (primary) | ≥0.60 | 0.650 | 300 | meets |
| negative precision | ≥0.60 | 1.000 | 10 | meets |
| negative recall — guards the above | ≥0.85 | 0.714 | 14 | **below** |
...
| negative | 10/14 = 71.4% | 10/10 = 100.0% |
| neutral | 238/252 = 94.4% | 238/253 = 94.1% |

**Key finding:** llama-side negative precision is 10/10 = 100.0% — when Llama says `negative`, humans usually see `neutral`. Llama over-calls negative; that is the systematic-bias pattern DESIGN.md asks this review to find.
...
|---|---|---|---|---|
| dev | 125 | 112/125 = 89.6% | 0.686 | 0.826 |
| holdout | 175 | 159/175 = 90.9% | 0.610 | 0.786 |
...
| gold_slice_6 | 42/50 = 84.0% (stratified — see headline note) |
...
Agreement 43/48 = 89.6%. DESIGN.md requires reading the gate stratified on this subgroup — the heuristic flags candidate rows; eyeball them:
...
| 2060 | 6 | gdelt | negative | neutral | Oppenheimer Downgrades Bank of America ( BAC ) to Perform |
...
| 23386 | 3 | gdelt | negative | neutral | Bandhan Bank Q1 results beat estimates , but stock crashes 15 %: Here what brokerages say on lender guidance cut |
...
| 17632 | 4 | gdelt | positive | neutral | Citizens Bank to cut ties with CoreCivic and GEO after a fierce public pressure campaign |
