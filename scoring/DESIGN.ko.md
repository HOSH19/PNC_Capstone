...
v3가 champion이고 **판정 기준 6개 중 2개 아래**다 — `negative` recall 0.714
(기준 0.85), `positive` recall 0.676(기준 0.70). 그럼에도 학습을 진행한다.
바를 세워놓고 넘지 못한 채로 가는 이유를 남긴다. 독자는 물을 자격이 있다.
...
v3가 놓치는 행도 완전한 무작위는 아니다 — "은행이 애널리스트 조치의 대상",
