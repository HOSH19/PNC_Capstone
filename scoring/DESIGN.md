...
| 2026-08-14 | **The 2020-2024 backfill no longer lives in Supabase; the backtest corpus is file-based.** The 401,652 GKG rows (plus 2,660 smoke-run scores) were deleted, and the historical path is now BigQuery → CSV → Kaggle GPU scoring → parquet, which the backtest reads directly. Supabase holds the live pipeline only. | The backfill pushed the free-tier database to 1,020 MB against a 500 MB quota — the attribution-flag backfill's 450k-row UPDATE doubled the heap and tripped Supabase into read-only mode, which also blocks the scheduled ingest and scoring jobs. Reclaiming space could not fix it: the data alone exceeded the quota. Deleting was cheap because the rows are **reproducible from a public dataset** (`backfill_gkg.py` rebuilds them inside BigQuery's free tier) and nothing derived from them existed yet beyond a smoke run. The cost is structural: one queue becomes two paths (live DB + backtest files), and `bank_sentiment_quarter` covers only the live window until the backtest merges its own aggregates. The alternative was $25/month for Pro; revisit if the team prefers paying over the split. After deletion and VACUUM FULL the database is 433 MB and read-only lifted itself. |
| 2026-08-14 | **Sentiment-side aggregation exists and is owned** (migrations 015/016, `pipeline/attribute_items.py`, `pipeline/aggregate_sentiment.py`, both scheduled in `score.yml`). The gate's verdict is persisted per row on `raw_item.attributed` — NULL means uncomputed, and a changed gate re-runs by nulling the affected rows — and `bank_sentiment_quarter` mirrors 012's contract (one writer; consumers read the table), keyed (bank_id, quarter) with the certificate carried for the join to `bank_index_score` and the distress labels. Directional counts are `p(class) >= threshold` with the threshold stored on every row, not argmax — obligation 2 made concrete. This closes the "assigned to nobody" flag in the attribution entry above: Jiwon owns it. | The gate is Python regex, so it cannot run inside a SQL rollup; persisting the verdict once per row is what lets the aggregation — and the backtest's threshold tuning, which needs item-level rows — be plain SQL. Attribution rates after the full backfill of the flag reproduced the gate's own measurements (99.4% GKG, 7.4% live-GDELT over the full period, 100% for ingest-matched sources). The verdict rules are per-source and closed: EDGAR and the enforcement/RSS feeds established the bank at ingest, so the title check would wrongly fail them; an unknown source raises. That guard fired on its first run and caught 2,405 rows of two sources (`alpha_vantage`, `newsapi`) present in the database but absent from the migrations' CHECK list — decided off-list and deleted, though how they were inserted past the constraint remains a team question. |
| 2026-08-14 | **The backtest corpus is both text sources, not just news** (`pipeline/backfill_edgar.py`; `corpus/edgar_2020_2024.csv`, 9,678 rows over 103 banks). The Kaggle notebook scores news and filings together and carries a `source` column so the axes can be weighted separately downstream. | The scoring contract has always been gdelt + edgar, and a sentiment axis backtested on half its serving diet would overweight news exactly where filings carry the signal density. Two per-bank findings worth keeping: **pnfp**'s seed CIK is a 2025 re-registration whose history is empty — its 2020-2024 filings live under the prior registrant (CIK 1115055, supplemented by hand); and **ozk** has no EDGAR filings at all because Bank OZK has no holding company and files with the FDIC under 12(i), not the SEC — a structural gap to state in the write-up, not a bug. |
| 2026-08-14 | **First combined backtest: the ladder beats both axes alone** (`evals/reports/2026-08-14_combined_ladder.md`). PR-AUC 0.081 combined vs 0.059 fundamentals-only vs 0.043 sentiment-only; recall at a 10-banks-per-quarter budget 9/32 vs 8/32 vs 6/32. Robust to the negative cut across 0.05–0.20. Starting cutoffs stay until the full grid (item threshold × negative cut × ladder lines) is swept. | The project's central claim measured for the first time, and it holds: sentiment is the weakest signal alone but complements fundamentals — the axes catch different events. Two honesty obligations for the write-up: 32 positives means differences are 1–2 events ("promising", not "proven"), and precision@50 trades away (0.04 vs 0.08) because the ladder pins Elevated-Risk quarters to the top of the ranking — tier interpretability costs raw top-k precision. |
| 2026-08-14 | **The combination rule is Rita's four-tier ladder, not a weighted average** (`pipeline/combine_axes.py`, implementing `compute_status` from dashboard PR #15, the mentor-agreed classification of 2026-07-12). Cutoffs ship as the dashboard's first-judgment values — the 30-point Imminent line, negative when a quarter's share of p≥0.5 negative items reaches 0.1 — all CLI flags, and **the backtest picks the final ones**. For the harness, risk_score is tier + distress_prob so the GP's probability orders banks within a tier without crossing tier boundaries. | The dashboard PR settled the how-to-combine question before it was ever argued: the ladder is implemented, mentor-derived, and matches how Rita renders status. One deliberate deviation — the **fundamentals floor** (score ≤ 80 is at least Elevated Risk regardless of sentiment). As written in the dashboard, score 20 with calm news rates *Watch* while the same score with no news rates *Elevated Risk*, so adding sentiment data improves a bank's rating; distress quarters are often quiet in the news and the labeler under-calls direction on top, so without the floor the ladder rates a pre-collapse bank better than ignorance would. Flagged to Rita for the dashboard rather than silently forked. |
| 2026-08-14 | **The fine-tuned weights are public**: GitHub Release `finbert-ft-2026-08-09` on the repo, which `score.yml` pins as `MODEL_RELEASE` and uses as its weight-cache key. The private Kaggle dataset of the same name stays for the GPU notebooks. | The scheduled scorer needed the weights without a third-party credential — the runner already holds `GITHUB_TOKEN`, and Kaggle was only ever storage for this path, not compute. Publishing was an explicit decision (the repo is public); training CSVs and `training_args.bin` are excluded from the release. Retraining means a new tag, which invalidates the workflow's cache by itself. |
